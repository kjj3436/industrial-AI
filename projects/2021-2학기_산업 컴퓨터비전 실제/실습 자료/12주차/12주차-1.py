...
pattern_size = (10,7)
samples = []
file_list = os.listdir('c:/img/pinhole_calib')
img_file_list = [file for file in file_list if file.startswith('img')]

for filename in img_file_list:
    
    frame = cv2.imread(os.path.join('c:/img/pinhole_calib', filename))
    ...
    cv2.putText(img_show, 'Samples captured: %d' % len(samples), (0, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 225, 0), 2)
...
rms, camera_matrix, dist_coefs, rvecs, tvecs = cv2.calivrateCamera(
